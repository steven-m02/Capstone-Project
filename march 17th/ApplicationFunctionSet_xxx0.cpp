...
void ApplicationFunctionSet::ApplicationFunctionSet_Obstacle() {
    static int forwardSpeed = 150;  // Speed for moving forward
    static int turnSpeed = 150;     // Speed for turning
    static int scanAngles[3] = {60, 90, 120};
    uint16_t distances[3] = {0};    // Store distances for left, center, right

    if (Application_SmartRobotCarxxx0.Functional_Mode == ObstacleAvoidance_mode) {
        uint16_t frontDistance;

        // Get front distance using the ultrasonic sensor
        AppULTRASONIC.DeviceDriverSet_ULTRASONIC_Get(&frontDistance);

        if (frontDistance < 15) {  // If an obstacle is closer than 15cm
            ApplicationFunctionSet_SmartRobotCarMotionControl(stop_it, 0);  // Stop robot
            delay(500);  // Small pause before scanning

            // Move the servo and scan Left, Center, and Right
            int scanAngles[3] = {30, 90, 150};  // Left, Center, Right positions
            for (int i = 0; i < 3; i++) {
                AppServo.DeviceDriverSet_Servo_controls(2, scanAngles[i]);  // Move servo to scan angle
                delay(500);  // Allow servo to stabilize before measuring
                AppULTRASONIC.DeviceDriverSet_ULTRASONIC_Get(&distances[i]);  // Measure distance
            }

            // Return the servo to the center position
            AppServo.DeviceDriverSet_Servo_controls(2, 90);
            delay(500);

            // Decide which way to move based on scanned distances
            if (distances[2] > distances[0] && distances[2] > 15) {  
                ApplicationFunctionSet_SmartRobotCarMotionControl(Right, turnSpeed);  // Turn Right
                delay(1000);  // Adjust time for a 90-degree turn
            } else if (distances[0] > distances[2] && distances[0] > 15) {
                ApplicationFunctionSet_SmartRobotCarMotionControl(Left, turnSpeed);  // Turn Left
                delay(1000);  // Adjust time for a 90-degree turn
            } else {
                ApplicationFunctionSet_SmartRobotCarMotionControl(Right, turnSpeed);  // Default to right turn
                delay(2000);  // Full 180-degree turn if both sides are blocked
            }

            // Resume moving forward
            ApplicationFunctionSet_SmartRobotCarMotionControl(Forward, forwardSpeed);
        } else {
    ...
    if (false == function_xxx(ULTRASONIC_Get, 0, 20)) //There is no obstacle 20 cm ahead?
    {
      ...
      if (timestamp == 3)
      {
        if (Position_Servo_xx != Position_Servo) //Act on servo motor：avoid loop execution
        {
          ...
          {
            time_Servo = millis();
            AppServo.DeviceDriverSet_Servo_controls(2, 80 /*Position_angle*/);
          }
          else if (Position_Servo == 2)
          {
            time_Servo = millis();
            AppServo.DeviceDriverSet_Servo_controls(2, 20 /*Position_angle*/);
          }
          ...
          {
            time_Servo = millis();
            AppServo.DeviceDriverSet_Servo_controls(2, 150 /*Position_angle*/);
          }
        }
      }
      else
 ...
 case 3 ... 4:
{
    ...
    AppServo.DeviceDriverSet_Servo_controls(2, angle);  // Now using Servo 2
}
